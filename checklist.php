...
if ($parent_id) {
    ...
    //echo "<p><strong>Componente Radice:</strong> $componente_radice</p>";
} else {
...
// Stampa ID della checklist e altri dettagli
//echo "<p><strong>Checklist ID:</strong> {$checklist['id']}</p>";
//echo "<p><strong>Checklist Componente Progetto ID:</strong> {$checklist['checklist_componente_progetto_id']}</p>";
...
$domande_stmt = $conn->prepare("
    SELECT d.id AS domanda_id, d.testo AS domanda_testo, d.tipo_contenuto, d.tipo_risposta, d.valore_media_url,
           r.id AS risposta_id, r.testo AS risposta_testo, r.valore_data AS risposta_data, r.valore_media_url AS risposta_media_url,
...
<div class="full-screen-container">
    <div class="container mt-5">
        ...
        <form id="checklistForm" method="post" action="salva_risposte.php" enctype="multipart/form-data">
            ...
            <input type="hidden" name="nome_componente" value="<?= $componente ?>">
            ...
            <?php while ($domanda = $domande->fetch_assoc()): ?>
                <div class="question-block">
                    ...
                    <!-- <p><strong>Domanda ID:</strong> <?= htmlspecialchars($domanda['domanda_id'], ENT_QUOTES, 'UTF-8') ?></p> -->
                    <!-- <p><strong>Domanda Checklist Componente Progetto ID:</strong> <?= htmlspecialchars($domanda['domanda_checklist_componente_progetto_id'], ENT_QUOTES, 'UTF-8') ?></p>-->
                    <input type="hidden" name="domanda_checklist_componente_progetto_ids[<?= $domanda['domanda_id'] ?>]" value="<?= $domanda['domanda_checklist_componente_progetto_id'] ?>">
                    ...
                    <?php elseif ($domanda['tipo_risposta'] == 'multiplo'): ?>
                        <div class="input-group mb-3">
                            <input type="text"
                                   name="risposte_text[<?= $domanda['domanda_id'] ?>]"
                                   value="<?= htmlspecialchars($domanda['risposta_testo'], ENT_QUOTES, 'UTF-8') ?>"
                                   class="form-control"
                                   placeholder="Inserisci risposta testuale"
                                <?= !empty($domanda['risposta_testo']) ? 'readonly' : '' ?>>
                            <input type="file"
                                   name="risposte_media[<?= $domanda['domanda_id'] ?>]"
                                   class="form-control"
                                <?= !empty($domanda['risposta_media_url']) ? 'disabled' : '' ?>>
                        </div>
                        <?php if (!empty($domanda['risposta_media_url'])): ?>
